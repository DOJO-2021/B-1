...
//				System.out.println(bbs_category);
		...
		if (request.getParameter("submit").equals("新規投稿")) {
			if(bbs_category1 ==0) {
		...
		//「下書きテーブル」に追加
			...
			if(bbs_category1 ==0) {
				drDao.insert(new Draft(user_id, 0, bbs_title, bbs_details, bbs_pw, bbs_range, bbs_category1));

			RequestDispatcher dispatcher = request.getRequestDispatcher("/WEB-INF/jsp/bbs_top.jsp");
			dispatcher.forward(request, response);
			}
			else {
				drDao.insert(new Draft(user_id, 0, bbs_title, bbs_details, bbs_pw, bbs_range, bbs_category2));
		...
		else if (request.getParameter("submit").equals("下書き読込")) {
